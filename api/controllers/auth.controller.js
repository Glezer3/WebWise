...
import { sendEmail, generateEmailTemplateVerify, generateEmailTemplateReset } from "../utils/sendEmail.js";
...
export const signup = async (req, res, next) => {
    ...
    try {
        ...
        const verifyURL = `${req.protocol}://${req.get('host')}/verify/${newUser._id}/${verify_token.token}`;
        const message = `We have received an email verification request. Please use the link below to verify your email: \n\n ${verifyURL} \n\n ` +
...
export const verification = async (req, res, next) => {
    ...
    try {
        ...
        if(validUser.verified) return res.status(200).json("This email is already verified");

        const validToken = await Token.findOne({ userId: validUser._id, token: verify_token });
        if(!validToken) return next(errorHandler(400, "Invalid token"));
...
export const signin = async (req, res, next) => {
    ...
    try {
      const validUser = await User.findOne({$or: [{ email: email }, {username: email}]});
      ...
      if(!validUser.verified) {
        if(resend === "true") {
            ...
            const verifyURL = `${req.protocol}://${req.get('host')}/verify/${validUser._id}/${verify_token.token}`;
            const message = `We have received an email verification request. Please use the link below to verify your email: \n\n ${verifyURL} \n\n ` +
            `Please remember that this link for email verification will be valid only for 1 hour.`;
...
export const google = async (req, res, next) => {
    ...
    try {
        ...
        if(user) {
            ...
            const newUser = new User({ username: req.body.name.split(" ").join("").toLowerCase() + Math.random().toString(36).slice(-5), email: email, password: hashedRandomPassword, avatar: photo, verified: true });
            await newUser.save();
...
export const forgotPassword = async (req, res, next) => {
    ...
    try {
      ...
      const reset_token = jwt.sign({ email: validUser.email, id: validUser._id }, secret, {expiresIn: "10m"} );
      ...
      `Please remember that this link for password reset will be valid only for 10 minutes.`;
...
export const resetPassword = async (req, res, next) => {
    ...
    const validUser = await User.findOne({passwordResetToken: reset_token, passwordResetTokenExpires: {$gt: Date.now()}});  

    if(!validUser) return next(errorHandler(400, "Token is either invalid or has already expired"));
